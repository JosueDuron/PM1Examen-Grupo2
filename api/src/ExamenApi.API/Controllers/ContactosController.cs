...
namespace ExamenApi.API.Controllers
{
    ...
    public class ContactosController : ControllerBase
    {
        ...
        {
            if (usuarioId <= 0) return BadRequest("Debe proporcionar un ID de usuario valido.");
        ...
        {
            if (dto.UsuarioId <= 0) return BadRequest("El contacto debe estar asociado a un ID de usuario valido.");
        ...
        {
            var eliminado = await _service.SoftDeleteAsync(id);
            if (!eliminado) return NotFound($"No existe un contacto con el ID {id}");
            return Ok(new { message = "Contacto eliminado logicamente." });
        }
    }
}
